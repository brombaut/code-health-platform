...
def calculate_complexity_over_range(git_dir, file_name, revision_range):
    start_rev, end_rev = revision_range
    revs = git_interactions.read_revs_for(git_dir, file_name, start_rev, end_rev)
    complexity_by_rev = []
    for rev in revs:
        historic_version = git_interactions.read_version_matching(git_dir, file_name, rev)
...
def run(args):
    ...
    complexity_trend = calculate_complexity_over_range(args.git_dir, args.file, revision_range)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--git-dir', required=True, type=str, help='The local git project directory')
