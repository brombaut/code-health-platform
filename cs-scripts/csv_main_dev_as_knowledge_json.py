...
def parse_author_color(csv_row, project_path=None):
    author = csv_row[0]
    color = csv_row[1]
    return author, color
...
def parse_ownership(csv_row, project_path=None):
    module = csv_row[0]
...
def run(args):
    ...
    knowledge_structure["children"] = [c for c in knowledge_structure["children"] if c["name"] != "."]
